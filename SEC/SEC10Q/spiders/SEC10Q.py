...
from SEC10Q.items import SecScrapingItem
import os
import pandas as pd
import re

os.chdir("D:/SEC10Q/data/")
...
class Sec10qSpider(BaseSpider):
    name = 'SEC10Q'
    start_urls = ['https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' + CIK[i] + '&type=10-q&dateb=&owner=include&count=100' for i in range(len(CIK))]
    MapCIK = dict(zip(CIK,TIC))
#    start_urls = ['https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000718877&type=10-q&dateb=&owner=include&count=100']
